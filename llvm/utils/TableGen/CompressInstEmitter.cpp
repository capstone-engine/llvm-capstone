...
void CompressInstEmitter::emitCompressInstEmitter(raw_ostream &OS,
                                                  EmitterType EType) {
    PI->compressInstEmitterEmitCompressInstEmitter(OS, EType, Target, CompressPatterns);
}
...
void CompressInstEmitter::run(raw_ostream &OS) {
  ...
  switch (PrinterLLVM::getLanguage()) {
    ...
    default: llvm_unreachable("No such language, only valid languages are "
                              "PRINTER_LANG_CPP and PRINTER_LANG_CAPSTONE_C");
  }
