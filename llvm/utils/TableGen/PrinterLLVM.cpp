...
void PrinterLLVM::regInfoEmitMCRegClassesTable(
    std::string const TargetName,
    std::list<CodeGenRegisterClass> const &RegisterClasses,
...
// DecodeStatus'.)
void PrinterLLVM::decoderEmitterEmitCheck() const {
...
void PrinterLLVM::subtargetEmitGetMacroFusions(CodeGenTarget &TGT,
            std::string Target,
...
void PrinterLLVM::subtargetEmitGetSTIMacro(StringRef const &Value,
                                           StringRef const &FieldName) const {
...
void PrinterLLVM::subtargetEmitSchedClassTables(
    SchedClassTablesT &SchedTables, std::string const &Target,
    CodeGenSchedModels const &SchedModels) const {
...
void PrinterLLVM::subtargetEmitMCSubtargetInfoImpl(
    std::string const &Target, unsigned NumFeatures, unsigned NumProcs,
...
void PrinterLLVM::subtargetEmitDFAPacketizerClass(
    CodeGenTarget &TGT,
    std::string const &Target, std::string const &ClassName) const {
...
void PrinterLLVM::subtargetEmitClassDefs(std::string const &Target,
                                         std::string const &ClassName,
...
void PrinterLLVM::instrInfoEmitMCInstrDescClose() const {
  OS << "  }, {\n";
}

void PrinterLLVM::instrInfoEmitMCInstrDescEnd() const {
  OS << "  }\n};\n\n";
}

void PrinterLLVM::instrInfoEmitMCInstrImplUses(
...
void PrinterLLVM::instrInfoEmitUseDefsLists(
    ...
    OS << TargetName << "ImpOpBase + " << EmittedLists[ImplicitOps]
       << ",\t";
}
...
void PrinterLLVM::instrInfoEmitOperandInfo(OperandInfoListTy &OperandInfoList) const {
  unsigned Offset = 0;
...
void PrinterLLVM::instrInfoEmitMCInstrDescDecl(
  std::string const &TargetName,
...
void PrinterLLVM::instrInfoEmitOpTypeOffsetTable(
  ...
  OS << "  static const " << getMinimalTypeForRange(OpRecSize);
  OS << " Offsets[] = {\n";
...
void PrinterLLVM::instrInfoEmitMacroDefineCheck() const {
  OS << "#if (defined(ENABLE_INSTR_PREDICATE_VERIFIER) && !defined(NDEBUG)) "
     << "||\\\n"
     << "    defined(GET_AVAILABLE_OPCODE_CHECKER)\n"
     << "#define GET_COMPUTE_FEATURES\n"
...
void PrinterLLVM::instrInfoEmitRequiredFeatureRefs(
    std::vector<std::vector<Record *>> const &FeatureBitsets,
    std::map<Record *, SubtargetFeatureInfo, LessRecordByID> const
        &SubtargetFeatures,
    CodeGenTarget const &Target) const {
  OS << "  static constexpr " << getMinimalTypeForRange(FeatureBitsets.size())
...
void PrinterLLVM::instrInfoEmitOpcodeChecker() const {
...
void PrinterLLVM::instrInfoEmitPredicateVerifier(StringRef const &TargetName) const {
  ...
  OS << "  FeatureBitset MissingFeatures =\n"
     ...
     << "    Msg << \"Attempting to emit \" << &" << TargetName
     << "InstrNameData[" << TargetName << "InstrNameIndices[Opcode]]\n"
     << "        << \" instruction but the \";\n"
...
void PrinterLLVM::compressInstEmitterEmitCompressInstEmitter(raw_ostream &OS, EmitterType EType,
 CodeGenTarget &Target, SmallVector<CompressPat, 4> &CompressPatterns) {
  // CodeGenTarget &Target = *CITarget;
  // SmallVector<CompressPat, 4> &CompressPatterns = *CICompressPatterns;
  ...
  for (auto &CompressPat : CompressPatterns) {
    ...
    compressInstEmitterGetReqFeatures(FeaturesSet, AnyOfFeatureSets, CompressPat.PatReqFeatures);
    ...
    compressInstEmitterGetReqFeatures(FeaturesSet, AnyOfFeatureSets, ReqFeatures);
    ...
    for (const auto &DestOperand : Dest.Operands) {
      ...
      case OpData::Operand: {
        ...
        if (DestOperand.Rec->isSubClassOf("RegisterClass") ||
          ...
          if (CompressOrUncompress) {
            unsigned Entry =
                compressInstEmitterGetPredicates(MCOpPredicateMap, MCOpPredicates, DestOperand.Rec,
                              "MCOperandPredicate");
            CondStream.indent(6)
                << ValidatorName << "("
            ...
            unsigned Entry =
                compressInstEmitterGetPredicates(ImmLeafPredicateMap, ImmLeafPredicates,
      ...
      case OpData::Imm: {
        if (CompressOrUncompress) {
          unsigned Entry = compressInstEmitterGetPredicates(MCOpPredicateMap, MCOpPredicates,
          ...
          unsigned Entry = compressInstEmitterGetPredicates(ImmLeafPredicateMap, ImmLeafPredicates,
                                         DestOperand.Rec, "ImmediateCode");
          CondStream.indent(6)
              << TargetName
    ...
    compressInstEmitterMergeCondAndCode(CaseStream, CondStream.str(), CodeStream.str());
    PrevOp = CurOp;
  }
  Func << CaseStream.str() << "\n";
  ...
  if (!MCOpPredicates.empty()) {
    ...
    compressInstEmitterPrintPredicates(MCOpPredicates, "MCOperandPredicate", OS);
  ...
  if (!ImmLeafPredicates.empty()) {
    ...
    compressInstEmitterPrintPredicates(ImmLeafPredicates, "ImmediateCode", OS);
...
void PrinterLLVM::compressInstEmitterPrintPredicates(const std::vector<const Record *> &Predicates,
...
void PrinterLLVM::compressInstEmitterMergeCondAndCode(raw_ostream &CombinedStream, StringRef CondStr,
                                                  StringRef CodeStr) {
...
unsigned PrinterLLVM::compressInstEmitterGetPredicates(DenseMap<const Record *, unsigned> &PredicateMap,
...
void PrinterLLVM::compressInstEmitterGetReqFeatures(std::set<std::pair<bool, StringRef>> &FeaturesSet,
               std::set<std::set<std::pair<bool, StringRef>>> &AnyOfFeatureSets,
